...
class Embeddings(nn.Module):
    def __init__(self, config, img_size, in_channels=3):
        ...
        n_patches = ((img_size[0] - patch_size[0]) // config.stride + 1) * ((img_size[1] - patch_size[1]) // config.stride + 1)
...
def ContrastiveLoss(features, labels, alpha=0.4):
    B, _ = features.shape
    features = F.normalize(features)

    sim_matrix = features.mm(features.t())

    sign_matrix_1 = torch.stack([labels == labels[i] for i in range(B)]).float()
    sign_matrix_2 = 1 - sign_matrix_1

    con_matrix_1 = 1 - sim_matrix
    con_matrix_2 = sim_matrix - alpha
    con_matrix_2[con_matrix_2 < 0] = 0

    loss = (con_matrix_1 * sign_matrix_1).sum() + \
           (con_matrix_2 * sign_matrix_2).sum()
    loss /= (B * B)
...
class ViT(nn.Module):
    def __init__(self, config, img_size=448, num_classes=200, zero_head=False):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            cross_loss = CrossEntropyLoss()(part_logits.view(-1, self.num_classes), labels.view(-1))
            con_loss = ContrastiveLoss(part_tokens[:, 0], labels.view(-1))
    ...
    def load_from(self, weights):
        # Read pretrained model, from ViT source code (https://github.com/jeonsworld/ViT-pytorch)
        with torch.no_grad():
